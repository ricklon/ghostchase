...
int tiltStatusFB;  //0 = left, 1 = middle ,2 = right 
...
int gameIntro(Adafruit_ST7735 tft) {
  ...
  while (1) {
    ...
    drawString(10, 50,"Yikes",ST7735_RED,2,tft);
    drawString(25, 65,"Real",ST7735_RED,2,tft);
    drawString(40, 80,"Ghosts!",ST7735_RED,2,tft);
...
int gamePlay(Adafruit_ST7735 tft) {
  ...
  int lastXX = xx;
  int lastYY = yy;
  int winXX =random(tft.height());
  int winYY =  random(tft.width());
  ...
  tft.fillScreen(ST7735_BLACK); 
  long prevTime = millis();
  long origTime = millis();
  placeGhosts(numGhosts,tft);
  //Place Win
  tft.fillRect(winXX, winYY,  15,15,ST7735_GREEN);

  while(1) {
    gTime = millis();
    tiltStatus = getTilt();
    tiltStatusFB = getTiltFB();
    if ((gTime - origTime) > 15000) 
    {
      break;
    }
    if ((gTime - prevTime) > 5) 
    {
      prevTime = millis();
      drawHealth(playerHealth, tft);
      tft.fillRect(0,20, 15,15,ST7735_BLACK);
      drawString(0, 20,tiltStatusFB,ST7735_RED,2,tft);
      if ((abs(xx - winXX) < 5) && (abs(yy - winYY) < 5))
      {
        return GAME_WIN;
      }
      if ( lastXX == xx && lastYY ==yy)
      {
        tft.fillRect(lastXX, lastYY, 15, 15, ST7735_BLACK);
      }
...
int gameDeath(Adafruit_ST7735 tft) {
  tft.fillScreen(ST7735_BLACK);
  drawString(0, 0,"SoulDeath!",ST7735_RED,2,tft);
  ...
  while (1) {
    ...
    placeGhosts(numGhosts*3,tft);
    if (gTime - prevTime > 8000) {
      break;
    }
  }
  return GAME_RESTART;
...
int gameWin(Adafruit_ST7735 tft) {
  ...
  for (ii = 0; ii < 90; ii = ii+10)
  { 
    tft.fillRect(ii-10, 90, 15,15,ST7735_BLACK);
    bmpDraw("ghost.bmp", ii+40, 61, tft);
    delay(20);
    bmpDraw("guy.bmp", ii, 90, tft);
    delay(100);
  }   
  delay(4000);
  return GAME_RESTART;
...
int gameRestart(Adafruit_ST7735 tft) {
  tft.fillScreen(ST7735_BLACK); 
  drawString(0, 0,"Start-->",ST7735_RED,2,tft);
  drawString(15, 20,"Refocus!",ST7735_RED,2,tft);
...
int getTiltFB() {
  bool AA = digitalRead(TILT_PINC);
  bool BB = digitalRead(TILT_PIND);
...
{
  drawString(10, 0, "Ghost", ST7735_RED,2,tft); 
...
void drawHealth(int amount, Adafruit_ST7735 tft)
{
  ...
  {
    bmpDraw("heart.bmp", ii*16, (tft.height() - 15), tft);
  }
}
...
void placeGhosts(int amount, Adafruit_ST7735 tft)
{
  for (ii = 0; ii < amount; ii++)
  {
    int xx = random(tft.width());
    int yy = random(tft.height());
    bmpDraw("sghost.bmp", xx, yy, tft);
  }
}
